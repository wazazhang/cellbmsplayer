package com.net.minaimpl.server.file;
...
//	----------------------------------------------------------------------------------------------------------------------
	...
	private FileSessionImpl getBindSession(IoSession session)
	{
		session_rw_lock.readLock().lock();
		try {
			Object obj = session.getAttribute(SessionAttributeKey.CLIENT_SESSION);
			if (obj instanceof FileSessionImpl) {
				return (FileSessionImpl) obj;
			}
		} finally {
	...
	public Iterator<TelnetSession> getSessions() {
		...
		try{
			...
			for (IoSession session : Acceptor.getManagedSessions().values()){
				FileSessionImpl client = getBindSession(session);
				if (client!=null){
					sessions.add(client);
				}
			}
...
//	-----------------------------------------------------------------------------------------------------------------------
	...
	public void sessionOpened(IoSession session) throws Exception {
		...
		FileSessionImpl client = new FileSessionImpl(session, this);
